...
@section('content')
    <div class="grid grid-cols-3 gap-4">
        <div class="p-8 bg-gray-200 col-span-1">
            <div class="flex flex-col">
                ...
                @foreach ($course->posts as $post)
                    <li class="flex items-center text-gray-600 mt-2">{{ $post->name }}
                        @if ($post->free)
                            <span
        ...
        <div class="text-gray-700 col-span-2">
            ...
            <div class="flex mt-3">
                <img src="{{ $course->user->avatar }}" class="mr-2 h-10 w-10 rounded-full" alt="">
            </div>
            <div class="">
                <p class="text-gray-500 text-sm">{{ $course->user->name }}</p>
                <p class="text-gray-300 text-xs">{{ $course->created_at->diffForHumans() }}</p>
            </div>
            <h2 class="text-center mt-4 mb-5">Ultimos Cursos</h2>
